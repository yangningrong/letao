...
$(function(){
  ...
  //1 一进入页面 发送请求  渲染页面
    ...
    //3 通过事件委托  给多有的dropdown里面的 a 添加点击事件
    ...
    $('#fileupload').fileupload({
      dataType:"json",
        ...
        console.log(data);
        var picObj = data.result; // 后台返回的结果
        ...
        if (picArr.length > 3 ){
          ...
          //移除数组最后一项
          picArr.pop();
          //移除图片结构中最后一个图片,找最后一个图片类型的元素,进行删除,让其自杀
          $("#imgBox img:last-of-type").remove();
        }

        if ( picArr.length === 3 ){
          //说明文件上传满3张了  picStatus 状态应该更新成VALID
    ...
    $('#form').on("success.form.bv",function(e){
      ...
      var params = $('#form').serialize(); //获取所有的input中的数据
